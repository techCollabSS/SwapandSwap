...
//  SwApp
//
//  Created by Juan Alvarez on 11/24/21.
//

// Use to Authenticate users in Firebase
//Use to handle User accounts in Firebase
...
class AuthService {
    ...
    func logOutUser(handler: @escaping (_ success: Bool) -> ()) { // NOTE: @escaping hanlder is used when we go from firebase and back
        
        do {
            try Auth.auth().signOut()
        } catch { // NOTE: This is to catch errors on a do try function.
            print("Error \(error)")
            handler(false)
            return
        }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            ...
            defaultsDictionary.keys.forEach { (key) in
                UserDefaults.standard.removeObject(forKey: key) // NOTE: This is the safe to make sure we are deleting all userDefaults after log out
            }
        }
    }
